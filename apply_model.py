...
def get_effect_of_this_catcat_from_relevances(reles, model, cols):
 ...
 skeys1 = get_sorted_keys(model, col1)
 skeys2 = get_sorted_keys(model, col2)
 postmultmat = []
 for key1 in skeys1:
...
def get_effect_of_this_catcont_from_relevances(reles, model, cols, boringValue=1):
 col1, col2 = cols.split(' X ')
 skeys = get_sorted_keys(model, col1)
 postmultmat = []
 for key in skeys:
...
def get_effects_of_interxns_from_relevance_dict(releDict, model):
 opDict = {}
 for col in model['catcats']:
  opDict[col] = get_effect_of_this_catcat_from_relevances(releDict[col],model, col)
 for col in model['catconts']:
  opDict[col] = get_effect_of_this_catcont_from_relevances(releDict[col],model,col)
 for col in model['contconts']:
  opDict[col] = get_effect_of_this_contcont_from_relevances(releDict[col],model,col)
 return opDict
